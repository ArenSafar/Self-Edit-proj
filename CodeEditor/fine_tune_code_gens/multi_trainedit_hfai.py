...
def train(dataloader, model, optimizer, scheduler, loss_scaler, epoch, local_rank, start_step, best_acc, save_path, args):
    ...
    for step, train_data in enumerate(dataloader):
        ...
        token_labels = token_labels.cuda()
        attn_mask = (token_labels.clone().detach() != 0).to(args.device,dtype=torch.uint8,non_blocking=True)
        loss_mask = (token_labels.clone().detach() == 2).to(args.device,dtype=torch.uint8,non_blocking=True)
        ...
        if step % args.logging_steps == 0:
            ...
            print("rank: %s, step: %s, batch_size: %s, len(dataloader): %s, loss: %s", args.local_rank, step, inputs.shape[0], len(dataloader), loss.item())
...
def main(local_rank, args):
    ...
    dist.init_process_group(backend='nccl', init_method='tcp://localhost:12345', world_size=gpus, rank= local_rank)
...
if __name__ == "__main__":
    ...
    print(ngpus)
    mp.spawn(main, args=(args, ), nprocs=ngpus)
    # main(0,args)