...
def complete_code(pipe, prompt, num_completions=1, max_length=256, **gen_kwargs):
    ...
    # print("MODEL CONFIG:", pipe.model.config)
    try:
...
def make_generations(dataset, args, model, tokenizer):
    ...
    print("Number of tasks is ", n_tasks)
...
def main(args):
    ...
    dataset = load_dataset(DATA_PATH, split="test", difficulties=[args.difficulty], trust_remote_code=True)
    ...
    if args.use_solutions:
        ...
        model = AutoModelForCausalLM.from_pretrained(args.model_ckpt, trust_remote_code=True)
        generations = make_generations(dataset, args, model, tokenizer)
        
    metric = load("loubnabnl/apps_metric", trust_remote_code=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--top_k", default=50, type=float, help="top k for sampling")
    ...
    parser.add_argument("--num_tasks", type=int, help="number of tasks to evaluate")
